...
export default function SecureFamilySchool() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50" dir="rtl">
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-lg border-b border-gray-200"><div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between"><div className="flex items-center gap-3"><ChildAvatar learner={learner} size="small" /><div><h1 className={`font-black ${learner.theme.text} arabic-text`}>مدرسة {learner.nameAr}</h1><p className="text-xs text-gray-500 arabic-text">المستوى {stats.level} • {stats.xp} XP • 🪙 {availableCoins}</p></div></div><div className="flex items-center gap-1"><button onClick={() => setShowRewards(true)} className="p-2.5 rounded-xl hover:bg-amber-50 text-amber-600" title="المكافآت"><Gift className="w-5 h-5" /></button>{selectedSubject && <button onClick={() => setSelectedSubject(null)} className="p-2.5 rounded-xl hover:bg-gray-100 text-gray-600"><Home className="w-5 h-5" /></button>}<button onClick={logout} className="p-2.5 rounded-xl hover:bg-gray-100 text-gray-500" title="قفل الحساب"><LogOut className="w-5 h-5" /></button></div></div></header>
