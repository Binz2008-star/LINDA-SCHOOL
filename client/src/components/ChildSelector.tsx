  ...
  return (
    ...
    >
      ...
      <div className={`mt-3 inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-white/70 ${child.colorText} border ${child.colorBorder}`}>
        <span>🌱</span>
        <span className="mr-1 arabic-text">{getLearningStageLabel(child.learningStage)}</span>
      </div>
    </motion.button>
  );
}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 flex flex-col items-center justify-center px-4 py-10">
      ...
      >
        ...
        <p className="text-gray-600 arabic-text text-base max-w-lg">
          لكل واحد منكم طريقته الخاصة. نبدأ من الأساس بهدوء، من دون مقارنة أو إحراج.
        </p>
      </motion.div>
      ...
      <motion.p
        ...
        className="mt-8 text-xs text-gray-500 arabic-text text-center"
        dir="rtl"
      >
        الخطأ هنا جزء من التعلّم، وليس سبباً للخجل 💙
      </motion.p>
    </div>
  );
}
