  ...
  return (
    <div className="max-w-2xl mx-auto" dir="rtl">
      <div className={`rounded-2xl bg-gradient-to-r ${learner.theme.gradient} text-white p-5 mb-5 text-center`}>
        <p className="text-lg font-black arabic-text mb-1">👆 اضغط على كل بطاقة وتعرّف عليها</p>
      ...
      {allTapped
        ? <motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onClick={onDone}
            ...
            <Zap className="w-4 h-4" /> انتقل للمراجعة
          </motion.button>
        : <button onClick={onDone} className="w-full mt-2 py-2 rounded-xl text-gray-400 text-sm arabic-text hover:text-gray-600">تخطي النشاط</button>
      }
    </div>
  );
}
...
function MathCountActivity({ learner, lesson, onDone }: {
  ...
  return (
    <div className="max-w-md mx-auto" dir="rtl">
      <div className={`rounded-2xl bg-gradient-to-r ${learner.theme.gradient} text-white p-5 mb-5 text-center`}>
        <p className="text-lg font-black arabic-text mb-1">🔢 نشاط العد</p>
        <p className="text-white/80 text-sm arabic-text">{lesson.activity}</p>
      </div>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 text-center">
        <p className="text-gray-600 arabic-text mb-4">اضغط + حتى تصل إلى <span className="font-black">{TARGET}</span></p>
        ...
        {count === TARGET && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-green-600 font-black arabic-text mb-2">✅ أحسنت!</motion.p>
        )}
      </div>
...
function EnglishMatchActivity({ learner, lesson, onDone }: {
  ...
  return (
    <div className="max-w-md mx-auto" dir="rtl">
      <div className={`rounded-2xl bg-gradient-to-r ${learner.theme.gradient} text-white p-5 mb-5 text-center`}>
        <p className="text-lg font-black arabic-text mb-1">🔤 طابق الكلمة الإنجليزية</p>
        <p className="text-white/80 text-sm arabic-text">اضغط البطاقة لتسمع النطق وترى المعنى</p>
      </div>
  ...
  return (
    <div className="max-w-lg mx-auto" dir="rtl">
      <div className={`rounded-2xl bg-gradient-to-r ${learner.theme.gradient} text-white p-5 mb-5`}>
        <p className="text-lg font-black arabic-text mb-1">✍️ نشاط عملي</p>
        <p className="text-white/90 text-sm arabic-text leading-relaxed">{lesson.activity}</p>
      </div>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-3">
        <p className="text-xs text-gray-400 arabic-text mb-2">رتّب الكلمات لتكوين الفكرة:</p>
        <div className="min-h-[44px] flex flex-wrap gap-2 p-2 bg-gray-50 rounded-xl mb-3 border-2 border-dashed border-gray-200">
          ...
          {placed.length === 0 && <span className="text-gray-300 text-sm arabic-text self-center">اضغط الكلمات أدناه…</span>}
        </div>
      ...
      <div className="flex gap-3">
        <button onClick={removeLast} className="px-4 py-2.5 rounded-xl bg-gray-100 text-gray-600 text-sm font-bold arabic-text hover:bg-gray-200">← حذف</button>
    ...
    <div className="flex items-center gap-1.5">
      {STEPS.map((_, i) => (
        <div key={i} className={`h-2 rounded-full transition-all ${
          i < stepIdx ? `w-6 bg-gradient-to-r ${learner.theme.gradient} opacity-60` :
    ...
    <div className="max-w-3xl mx-auto" dir="rtl">
      <div className="flex items-center justify-between mb-5">
        <button onClick={onBack} className="flex items-center gap-2 text-gray-600 arabic-text">
          <ArrowRight className="w-5 h-5" /> الدروس
        </button>
        <StepDots />
      </div>
        ...
        <div className="p-6 md:p-9">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              ...
              {previous && <span className="inline-flex mt-2 px-3 py-1 rounded-full bg-green-50 text-green-700 text-xs font-bold arabic-text">تمت دراسته سابقاً ✓</span>}
            </div>
          </div>
          ...
          <section className="rounded-2xl bg-blue-50 p-5 mb-5">
            <h2 className="font-black text-blue-950 arabic-text mb-3 flex items-center gap-2">
              <BookOpen className="w-5 h-5" /> الشرح
            </h2>
          ...
          <section className={`rounded-2xl ${learner.theme.light} p-4 mb-5`}>
            <p className={`text-xs font-bold ${learner.theme.text} arabic-text mb-1`}>مثال</p>
          ...
          <section className="rounded-2xl bg-amber-50 border border-amber-100 p-4 mb-6">
            <p className="text-xs font-bold text-amber-700 arabic-text mb-0.5 flex items-center gap-1">
              <Lightbulb className="w-3 h-3" /> تذكّر
            </p>
          ...
          <button onClick={() => setPhase('activity')}
            ...
            <Zap className="w-5 h-5" /> انتقل للنشاط العملي
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="max-w-3xl mx-auto" dir="rtl">
      <div className="flex items-center justify-between mb-5">
        <button onClick={() => setPhase('teach')} className="flex items-center gap-2 text-gray-600 arabic-text">
          <ArrowRight className="w-5 h-5" /> الشرح
        </button>
        <StepDots />
      </div>
    ...
    <div className="max-w-2xl mx-auto" dir="rtl">
      <div className="flex items-center justify-between mb-5">
        <button onClick={() => setPhase('activity')} className="flex items-center gap-2 text-gray-600 arabic-text">
          <ArrowRight className="w-5 h-5" /> النشاط
        </button>
        <StepDots />
      ...
      <motion.div key={questionIndex} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between mb-4">
          <span className={`text-xs font-bold px-3 py-1 rounded-full ${learner.theme.light} ${learner.theme.text} arabic-text`}>مراجعة سريعة</span>
          <button onClick={() => speak(questionText, { lang: 'ar', rate: 0.82 })}
            className="p-2 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors" title="اسمع السؤال">
          ...
          {question.options.map((option, index) => {
            ...
            return (
              <button key={option} disabled={answered} onClick={() => selectAnswer(index)}
                className={`w-full min-h-[54px] rounded-xl border-2 p-4 text-right flex gap-3 arabic-text transition-all ${
                  correct ? 'bg-green-50 border-green-500 text-green-950' :
                ...
                <span className={`w-8 h-8 rounded-full flex items-center justify-center font-black flex-shrink-0 ${
                  correct ? 'bg-green-500 text-white' : wrong ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-600'}`}>
          ...
          {answered && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <p className={`text-sm font-black arabic-text mb-2 ${isCorrect ? 'text-green-800' : 'text-amber-800'}`}>
                {isCorrect ? '✅ إجابة صحيحة!' : '💡 لنفهم الإجابة:'}
              </p>
              {tutorLoading
              ...
              <button onClick={nextQuestion}
                ...
                {questionIndex < totalQ - 1 ? 'السؤال التالي ←' : '🏁 انهِ الدرس'}
              </button>
            </motion.div>
          )}
        </AnimatePresence>
  ...
  return (
    <div className="max-w-md mx-auto text-center" dir="rtl">
      <div className="bg-white rounded-3xl border border-gray-100 shadow-xl p-8">
        ...
        <h1 className="text-2xl font-black text-gray-900 arabic-text">أكملت الدرس يا {learner.nameAr}!</h1>
        <p className="text-gray-500 mt-2 text-sm arabic-text">شرح + نشاط + مراجعة ✓</p>
        ...
        <div className="flex flex-col gap-2">
          ...
          <button onClick={resetLesson}
            ...
            <RotateCcw className="w-4 h-4" /> أعد الدرس
          </button>
        </div>
      </div>
    </div>
